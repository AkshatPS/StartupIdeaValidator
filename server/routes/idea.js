...
router.post("/", auth, async (req, res) => {
  try {
    ...
    axios
      .post(`${process.env.FLASK_API_URL}/analyze`, {
    ...
    res
      .status(201)
      .json({
        message: "Idea submitted successfully and is being processed.",
...
router.get("/my-ideas", auth, async (req, res) => {
  try {
    const ideas = await Idea.find({ userId: req.user.id }).sort({
      createdAt: -1,
    });
    res.json(ideas);
  } catch (error) {
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    if (idea.userId.toString() !== req.user.id) {
      return res
        .status(403)
        .json({ message: "User not authorized to view this idea." });
    }
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    if (idea.userId.toString() !== req.user.id) {
      return res
        .status(403)
        .json({ message: "User not authorized to delete this idea." });
    }
...
router.put("/:id", auth, async (req, res) => {
  try {
    ...
    if (idea.userId.toString() !== req.user.id) {
      return res
        .status(403)
        .json({ message: "User not authorized to edit this idea." });
    }

    ...
    useEffect(() => {
        ...
        const fetchAndPoll = () => {
            const intervalId = setInterval(async () => {
                try {
                    ...
                    setError("Could not retrieve validation status. Please refresh the page.");
                    clearInterval(intervalId);
                }
        ...
        axios.get(`http://localhost:5000/api/ideas/${ideaId}`, config)
            ...
            .catch(err => {
                ...
                setError("Failed to load idea data. The idea may not exist or you may not have permission to view it.");
            });

    }, [ideaId]); // Re-run effect if the ideaId changes
        ...
        <div className="validation-page-wrapper">
            ...
            <div className="validation-container">
                ...
                <div className="sections-container">
                    {/* These sections will now appear dynamically based on the analysis results */}
                    <ValidationSection title="Market Fit Analysis" icon="📊" isVisible={analysisResult?.executiveSummary}>
                        <p>{analysisResult?.executiveSummary?.keyFindings?.join(' ')}</p>
                    </ValidationSection>

                    <ValidationSection title="Competitor Check" icon="⚔️" isVisible={analysisResult?.swotAnalysis}>
                         <p><strong>Threats:</strong> {analysisResult?.swotAnalysis?.threats?.join(', ')}</p>
                    </ValidationSection>

                    <ValidationSection title="Feasibility Score" icon="🎯" isVisible={analysisResult?.executiveSummary?.overallScore}>
                        <div className="feasibility-score-container">
                            <div className="score-display">
                                <span className="score-value">{analysisResult?.executiveSummary?.overallScore}</span>
                                <span className="score-total">/ 100</span>
                            </div>
                            <div className="score-breakdown">
                                <strong>Recommendation: {analysisResult?.executiveSummary?.recommendation}</strong>
                            </div>
                        </div>
                    </ValidationSection>
                    
                    <ValidationSection title="Feedback & Suggestions" icon="💡" isVisible={analysisResult?.nextSteps}>
                        <ul className="feedback-list">
                           {analysisResult?.nextSteps?.map((step, index) => <li key={index}>{step}</li>)}
                        </ul>
                    </ValidationSection>

                     <ValidationSection title="Final Summary Report" icon="📄" isVisible={ideaData.status === 'completed'}>
                        <div className="report-preview">
                            <h4>Validation Summary</h4>
                            <p><strong>Idea:</strong> {ideaData.title}</p>
                            <p><strong>Final Score:</strong> {analysisResult?.executiveSummary?.overallScore}/100</p>
                            <p><em>Validated on: {new Date(ideaData.createdAt).toLocaleDateString()}</em></p>
                        </div>
                        <button onClick={() => navigate(`/report/${ideaId}`)} className="btn btn-primary">View Full Report</button>
                    </ValidationSection>
                </div>

                <div className="page-actions">
                    <a href={`/edit-idea/${ideaId}`} className="btn btn-secondary">📝 Edit Idea</a>
                    <a href={`/report/${ideaId}`} className="btn btn-accent">📄 Download Full Report</a>
                </div>
            </div>
